...
class User {
  ...
  static async authenticate(username, password) { 
    const results = await db.query(
      `SELECT username, password 
       ...
       WHERE username = $1`,
      [username]);
    const authenticateUser = results.rows[0];
    if (authenticateUser && await bcrypt.compare(password, authenticateUser.password)) {
      return true;
    }
    else {
      return false;
  ...
  static async get(username) { 
    ...
    if (results.rows.length === 0) {
       throw new ExpressError(`The user: ${username} is not found`, 404);
    }
    return results.rows[0];
  }
  ...
  static async messagesFrom(username) {
    ...
    const msgUsers = results.rows.map(m => ({ 
      id: m.id, 
      body: m.body, 
      sent_at: m.sent_at, 
      read_at: m.read_at, 
  ...
  static async messagesTo(username) { 
    ...
    const usersMsg = results.rows.map(m => ({ 
      id: m.id, 
      body: m.body, 
      sent_at: m.sent_at, 
      read_at: m.read_at, 
